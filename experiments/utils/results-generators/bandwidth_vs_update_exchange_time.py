...
def compute_avg_exchange_time(data):
	averages = {}
	for bandwidth, entries in data.items():
		exchange_times = [entry["update_exchange_time"] for entry in entries if "update_exchange_time" in entry]
		if exchange_times:
			avg = sum(exchange_times) / len(exchange_times)
			averages[bandwidth] = avg
	return averages


def plot_horizontal_bar_chart(averages):
	bandwidths = sorted(averages.keys(), key=lambda d: int(d))
	times = [averages[bandwidth] for bandwidth in bandwidths]
	colors = ["#27AE60"]

	fig, ax = plt.subplots(figsize=(7.5, 3))
	y_pos = np.arange(len(bandwidths)) * 0.2

	bars = ax.barh(y_pos, times, color=colors, height=0.15)

	ax.set_yticks(y_pos)
	ax.set_yticklabels([f"{bandwidth} Mbps" for bandwidth in bandwidths])
	ax.set_title("Impact of the Bandwidth on Update Exchange Time", pad=15)
	...
	plt.tight_layout()
	output_path = "bandwidth_vs_update_exchange_time.png"
...
def validate_args(args: list[str]) -> str:
	if len(args) != 2:
		raise ValueError("Usage: python bandwidth_vs_update_exchange_time.py <path_to_experiment_result.json>")
	return args[1]
